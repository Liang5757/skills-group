...
def sample_dates(start_date, end_date, freq):
    ...
    if freq == "daily":
        d = start
        while d <= end:
            if d.weekday() < 5:  # 工作日；节假日由取数阶段回退处理
                out.append(d)
            d += timedelta(days=1)
    elif freq == "weekly":
        d = start
...
def fetch_shares(start_date, end_date, freq):
    ...
    for i, sd in enumerate(dates):
        ...
        for offset in [0, -1, -2, -3, -4]:
            ...
            try:
                ...
                if df is not None and "基金代码" in df.columns and len(df) > 0:
                    gs = group_shares(df)
                    if gs["全宽基合计"] > 0:
                        if try_str not in seen_dates:
                            results.append({"date": try_str, **gs})
                            seen_dates.add(try_str)
                        else:
                            duplicate += 1
                        found = True
                        if (i + 1) % 10 == 0 or i == 0:
        ...
        if not found:
            fail += 1
        time.sleep(0.3)
    print(f"完成！成功 {len(results)} 条，失败 {fail} 条，重复日期 {duplicate} 条")
    return results


def load_existing_data(json_path):
    """读取增量模式的既有时间序列；文件不存在时返回空列表。"""
    ...
    try:
        ...
        raise ValueError(f"无法读取既有 JSON: {json_path}: {ex}") from ex
    if not isinstance(data, list) or any(
            not isinstance(row, dict) or "date" not in row for row in data):
        raise ValueError(f"既有 JSON 格式无效: {json_path}")
    return data


def merge_by_date(existing, fresh):
    """按实际统计日期合并，避免节假日回退产生重复记录。"""
...
def make_charts(data, prices, out_dir):
    ...
    fig.suptitle("国家队各宽基 ETF 份额代理 vs 指数走势",
...
def main():
    ...
    ap.add_argument("--freq", default="weekly", choices=["daily", "weekly", "monthly"], help="采样频率")
    ap.add_argument("--incremental", action="store_true", help="保留已有 JSON，仅抓取并追加最新日期")
    ...
    existing = load_existing_data(json_path) if args.incremental else []
    fetch_start = args.start
    if existing:
        latest = max(datetime.strptime(row["date"], "%Y%m%d") for row in existing)
        requested_start = datetime.strptime(args.start, "%Y-%m-%d")
        fetch_start = max(requested_start, latest + timedelta(days=1)).strftime("%Y-%m-%d")
        print(f"增量模式：已有 {len(existing)} 条，最新 {latest:%Y-%m-%d}，从 {fetch_start} 继续")

    print(f"=== 获取全宽基 ETF 份额 ({fetch_start} ~ {args.end}, {args.freq}) ===")
    fresh = (fetch_shares(fetch_start, args.end, args.freq)
             if fetch_start <= args.end else [])
    data = merge_by_date(existing, fresh) if args.incremental else fresh
    if not data:
