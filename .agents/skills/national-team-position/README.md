...
- 追踪 ETF **份额**（而非规模，已排除价格涨跌干扰），更直接地反映 ETF 申赎变化
- 输出一张**六合一总图** + 六张**各指数单图**（每张 = 该宽基份额 + 对应指数价格双轴）+ JSON 数据
- 辅助观察「整体份额下降」与「宽基轮动」（例如沪深300 见顶后份额是否流向新发的中证A500）

### 示例输出

![六合一总图：各宽基 ETF 份额 vs 对应指数走势](assets/overview.png)

> 上图为 2023 至今的六合一总图：每格的彩色线是该宽基的 ETF 份额（左轴，作为国家队仓位代理），灰线是对应指数价格（右轴）。
...
| `--freq` | 采样频率 `daily` / `weekly`（默认）/ `monthly` |
| `--incremental` | 保留既有 JSON，仅抓取并追加最新日期，适合每日自动化 |
| `--data-only` | 仅输出 JSON，不画图 |

## 原理

脚本将宽基 ETF **份额**变化作为观察国家队仓位的代理指标。份额变化也包含其他机构和个人的申赎，不能单独证明中央汇金交易。脚本按日、周或月从
上交所 ETF 份额接口（经 [AKShare](https://github.com/akfamily/akshare) 封装的 `fund_etf_scale_sse`，
一次调用即返回当日全部 ETF）采样各宽基份额，并叠加各指数日线走势。日频模式仅生成周一至周五候选日期，
节假日由接口回退处理；自动化建议配合 `--incremental`，避免重复抓取历史数据。
